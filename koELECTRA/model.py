...
from transformers import ElectraConfig # KoELECTRA의 설정을 명시적으로 가져옵니다.
...
class Models():
    ...
    def BERT(self):
        if self.model_name == 'bert':       
        ...
        elif self.model_name == 'koelectra':
            # KoELECTRA 모델 설정 로드
            ...
            # ElectraForSequenceClassification으로 모델 로드
            self.model = model = ElectraForSequenceClassification.from_pretrained(
                                            "monologg/koelectra-base-discriminator", 
                                            config=config,
                                        )
            # ElectraTokenizer로 토크나이저 로드
